...
async def on_message(message, reply, loop=None, name=task.name, verbose=True, **kwargs):
    ...
    try:
        ...
        # Send message is now done from the inside of task
        await task.process_message(task_data, loop=loop, send_reply=send_reply, metadata=task_metadata, reject=message.reject, reply=reply, routing_key=routing_keys['finalResult'] ,producer_name=name,message=message,**kwargs)
        if verbose:
            print('Job for item %s completed!' % item)
        message.ack()

    except NoReply as e:
        print('Job for item %s finished via no reply:' % item, e)
